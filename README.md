...
You can download a beta release of I-DLV-sr for Linux x86-64 [here](https://github.com/DeMaCS-UNICAL/I-DLV-sr/releases/tag/v1.0).
...
It is possible to run the I-DLV-sr system in two different modes: **socket-based** and **file-based**.

#### **Socket-based**

The system reads the input stream from a source socket. Once running, it waits for input events until the socket is open.

In this mode, one should:
...
The system reads the input stream from a log file. Once running, it reads all input events up to the end of the file.

In this mode, one should just issue:
```
...
An I-DLV-sr rule can be of one out of the two forms:
1. α : - l<sub>1</sub>, ... , l<sub>b</sub>.
2. **\#temp** α : - l<sub>1</sub>, . . . , l<sub>b</sub>.

where:
* *α* is a predicate atom i.e., a standard [ASP-Core-2](https://arxiv.org/abs/1911.04326) atom of form *p(t<sub>1</sub>,...,t<sub>n</sub>)*
* *l<sub>1</sub>, ... , l<sub>b</sub>* is a conjunction of streaming literals

Given a predicate atom *α*, a constant *c ∈ N+*, a term *t*, and some natural numbers *D = {d<sub>1</sub>, ... , d<sub>m</sub>}*, we define three types of streaming atoms:
* *α **at least** c **in** {d<sub>1</sub>, ... , d<sub>m</sub>}*:
  holds if *α* is true *at least c* times within time points identified by *{d<sub>1</sub>, ... , d<sub>m</sub>}*;
* *α **always in** {d<sub>1</sub>, ... , d<sub>m</sub>}*:
  holds if *α* is *always* true within within time points identified by *{d<sub>1</sub>, ... , d<sub>m</sub>}*;
* *α **count** *t* **in** {d<sub>1</sub>, ... , d<sub>m</sub>}*:
  counts how many time *α* is true within within time points identified by *{d<sub>1</sub>, ... , d<sub>m</sub>}*.
...
Given a streaming atom of any type *s* and an evaluation time point *t<sup>e</sup>*, the set of natural numbers *D = {d<sub>1</sub>, ... , d<sub>m</sub>}* defines how we have to look backwards starting from *t<sup>e</sup>* when evaluating *s*, i.e, it will be evaluated within the time points defined by {t<sup>e</sup>-d<sub>1</sub>, ... ,t<sup>e</sup>-d<sub>m</sub>}. For instance, if *s = α **always in** {0, 1, 2, 3}* and *t<sup>e</sup>=10*, then *s* will be evaluated over the time points *{10, 9, 8, 7}*.

If *D = {n ∈ N | 0 ≤ n ≤ w ∧ w > 0}* we indicate it simply as **[w]**; e.g., we write *α **always in** [3]* instead of *α **always in** {0, 1, 2, 3}*.


For a streaming literal *s*, a streaming literal can be either the positive literal (*s*) or the negative (***not*** *s*), where ***not*** denotes negation as failure.

The following shortcuts are admitted:
...
It requires a stream log consisting of chronologically ordered elements of the form:
...
Each timestamp of a stream log represents a *time point*.
### Example
...
```
In this example, at the time point ```2020-05-26T12:16:25``` an irregularity is detected thanks to the second rule of the program.
Indeed, the head ```irregular``` is derived because all the streaming atoms within its body hold. In particular:
...
At the time point ```2020-05-26T12:16:25``` also ```numanomalies(1)``` is derived since the counting atom in its body ```irregular count X in [30]``` finds (and counts) the just derived ```irregular``` within a window of the last 30 seconds constructed starting from ```2020-05-26T12:16:25```.

**Note:** since the program does not define an explicit time unit for the streaming atoms, the system will use *seconds* by default.
...
## ICLP 2021 Submission - Experiments

The benchmarks used for the experimental analysis performed within the paper are available in the folder: **ICLP2021-Experiments**.
It contains, for each compared system ([Distributed-SR](http://distributed-stream-reasoner.ainf.at/prototype/) and I-DLV-sr) and for each test performed, the related *Programs* and *Logs*.
